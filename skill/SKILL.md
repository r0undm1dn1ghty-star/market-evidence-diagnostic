...
description: "Проверка бизнес-модели через рынок: вывод=источник+дата."
license: Apache-2.0
metadata:
  release: 0.3.0-rc1
...
Используй этот skill, когда запрос содержит фразы-маркеры:

- «проверь бизнес-модель» — нужна диагностика жизнеспособности на фактах рынка
- «выживет ли» — оценка, пройдёт ли модель контакт с рынком
- «есть ли спрос» — отделить сигнал спроса от self-claim и метрик активности
- «альтернативы» — карта текущих workaround, конкурентов и status quo

Также подходит для SaaS, сервисов, consumer-приложений, marketplace и open-source-продуктов перед запуском, новым сегментом, каналом, pricing change или ростом.

**Анти-триггеры — не использовать:**

- Общий консалтинг без проверки рынка (стратегия, прогнозы, совет «что делать дальше»).
- Прогноз выручки, PMF-вердикт или оценка успеха компании.
- Замена customer research, скрытый сбор контактов или рассылка.
- Юридическая, инвестиционная или финансовая консультация.
...
```bash
Правило единственного независимого источника (Рыночный прогон 20.09.2026): если из открытых источников доступен только 1 независимый locator, вердикт не поднимается выше `switch_hypothesis_testable`, а уверенность ограничивается пометкой в отчёте. Strict-валидатор отклонит такой pack — это не дефект, а видимая граница метода.

python scripts/validate_diagnostic.py <папка_диагностики>          # strict evidence gate (по умолчанию)
python scripts/validate_diagnostic.py --legacy <папка_диагностики> # старый структурный gate (для архивных pack'ов)
python scripts/test_adversarial.py                                 # 6 red-team сценариев против validator'а
```
