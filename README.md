...
> Market Evidence Diagnostic — open-source skill для проверки утверждений бизнес-модели через рынок. Research pack для AI-агентов и консультантов: реальная проблема, альтернативы, доказательства из каналов. Apache 2.0, v0.3.0-rc1.

> **Термин:** _claims_ в этом проекте означает **утверждения о бизнес-модели, которые подаются как факт, но не подтверждены доказательствами из независимых источников.** Русский аналог — «утверждения» (или «заявления»). Не путать с «гипотезами» — гипотеза признаётся непроверенной, а claim выдаётся за известное.
