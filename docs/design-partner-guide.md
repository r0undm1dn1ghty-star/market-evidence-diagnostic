...
| 3. Evidence gaps | При необходимости предоставляет минимальный обезличенный export под конкретный claim. | Показывает, какие claims уже поддержаны и что ещё блокирует verdict. | Hypothesis map и diagnostic report. |
...
Полезный run не обязан улучшить verdict. Он полезен, если он: разделил claim объекта и независимое evidence; выявил хотя бы один конкретный evidence gap; заблокировал необоснованный readiness claim; либо сделал workflow/template заметно проще и точнее для следующего пользователя.
